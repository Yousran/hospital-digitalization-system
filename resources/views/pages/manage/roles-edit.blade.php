    ...
    <div class="flex flex-col min-h-screen items-center justify-center px-6 py-8 mx-auto">
        ...
        <div class="w-full bg-white rounded-lg shadow dark:border sm:max-w-md xl:p-0 dark:bg-gray-800 dark:border-gray-700">
            <div class="p-6 space-y-4 md:space-y-6 sm:p-8">
                ...
                <form method="POST" action="{{ route('roles.update', $role->id) }}" class="space-y-4 md:space-y-6">
                    @csrf
                    @method('PUT')
                    <div class="grid grid-cols-1 md:grid-cols-1 gap-4">
                        ...
                        <div class="mb-4">
                            ...
                            <textarea name="description" class="mt-1 block w-full p-2 border rounded-lg">{{ $role->description ?? '' }}</textarea>
                        </div>
                        <div class="mb-4">
                            <label for="badge_colour" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Badge Colour</label>
                            <input type="text" name="badge_colour" class="mt-1 block w-full p-2 border rounded-lg" value="{{ $role->badge_colour }}">
                        </div>
                    </div>

                    <div class="flex space-x-4 w-full">
                        <!-- Cancel Button -->
                        <a href="{{ route('roles.index') }}" class="w-full text-gray-900 bg-gray-200 hover:bg-gray-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-gray-700 dark:text-white dark:hover:bg-gray-600 dark:focus:ring-gray-800">
                            Back
                        </a>
